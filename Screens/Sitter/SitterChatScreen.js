...
const SitterChatScreen = ({ route }) => {
    ...
    const handleSend = () => {
        const messagePost = {
            sender: route.params.sitter,
            receiver: route.params.username,
            message: textMessage
        }
        axios.post('http://localhost:3000/chat/addMessageSitter', messagePost)
            .then(res => {
                setallmessages(res.data[0].messages.reverse())
            })
            .catch(err => console.log(err))
    ...
    return (
        <View style={styles.container}>
            <FlatList style={styles.list}
                ...
                renderItem={({ item }) => (
                    <View style={item.sender == route.params.sitter?styles.sitterView:styles.senderView}>
